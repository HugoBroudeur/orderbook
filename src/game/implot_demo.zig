...
pub fn imPlotDemoTabs() !void {
    if (ig.igBeginTabBar("ImPlotDemoTabs", 0)) {
        ...
        if (ig.igBeginTabItem(ifa.ICON_FA_CHART_LINE ++ " Plots", null, 0)) {
        ...
        if (ig.igBeginTabItem(ifa.ICON_FA_CHART_AREA ++ " Subplots", null, 0)) {
            defer ig.igEndTabItem();
        ...
        if (ig.igBeginTabItem(ifa.ICON_FA_CHART_COLUMN ++ " Axes", null, 0)) {
            defer ig.igEndTabItem();
        ...
        if (ig.igBeginTabItem(ifa.ICON_FA_WRENCH ++ " Custom", null, 0)) {
            defer ig.igEndTabItem();
        ...
        for (0..@intCast((st.rows * st.cols))) |i| {
            if (ip.ImPlot_BeginPlot("", .{ .x = 0, .y = 0 }, ip.ImPlotFlags_NoLegend)) {
                ...
                if (st.rows * st.cols > 1) {
                    ...
                    // TODO
                    ip.ImPlot_SetNextLineStyle(.{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w }, utils_ip.IMPLOT_AUTO);
                ...
                for (0..6) |j| {
                    if (st.id[j] == i) {
                        ...
                        if (ip.ImPlot_BeginDragDropSourceItem(slLabel.ptr, 0)) {
                            ...
                            // TODO
                            ip.ImPlot_ItemIcon_Vec4(.{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w });
                            ig.igSameLine(0, -1.0);
...
fn demo_AutoFittingData() !void {
    ...
    ig.igSameLine(0, -1.0);
    _ = ig.igCheckboxFlags_IntPtr("ImPlotAxisFlags_AutoFit##X", &st.xflags, ip.ImPlotAxisFlags_AutoFit);
    ig.igSameLine(0, -1.0);
    _ = ig.igCheckboxFlags_IntPtr("ImPlotAxisFlags_RangeFit##X", &st.xflags, ip.ImPlotAxisFlags_RangeFit);

    ig.igTextUnformatted("Y: ", "\n");
    ig.igSameLine(0, -1.0);
    _ = ig.igCheckboxFlags_IntPtr("ImPlotAxisFlags_AutoFit##Y", &st.yflags, ip.ImPlotAxisFlags_AutoFit);
    ig.igSameLine(0, -1.0);
...
fn demo_OffsetAndStride() !void {
    ...
    if (ip.ImPlot_BeginPlot("##strideoffset", .{ .x = -1, .y = 0 }, ip.ImPlotFlags_Equal)) {
        ...
        for (0..k_circles) |ci| {
            ...
            ipz.ImPlot_PlotLineXyEx(&buff, interleaved_data[ci * 2 + 0 ..].ptr, interleaved_data[ci * 2 + 1 ..].ptr, k_points_per, 0, st.offset, 2 * k_circles * @sizeOf(@TypeOf(interleaved_data[0])));
        }
        ip.ImPlot_EndPlot();
...
fn demo_DragPoints() !void {
    ...
    if (ip.ImPlot_BeginPlot("##Bezier", .{ .x = -1, .y = 0 }, ip.ImPlotFlags_CanvasOnly)) {
        ...
        // TODO
        //ipz.ImPlot_PlotLineXyEx("##h1",&st.P[0].x, &st.P[0].y, 2, 0, 0, @sizeOf(ip.ImPlotPoint));
        ip.ImPlot_SetNextLineStyle(.{ .x = 0, .y = 0.5, .z = 1, .w = 1 }, if (st.hovered[2] or st.held[2]) 2.0 else 1.0);
        // TODO
        //ipz.ImPlot_PlotLineXyEx("##h2",&st.P[2].x, &st.P[2].y, 2, 0, 0, @sizeOf(ip.ImPlotPoint));
        ip.ImPlot_SetNextLineStyle(.{ .x = 0, .y = 0.9, .z = 0, .w = 1 }, if (st.hovered[0] or st.held[0] or st.hovered[3] or st.held[3]) 3.0 else 2.0);
        // TODO
        //ipz.ImPlot_PlotLineXyEx("##bez",&B[0].x, &B[0].y, 100, 0, 0, @sizeOf(ip.ImPlotPoint));
        ip.ImPlot_EndPlot();
    }
}
...
fn demo_Images() !void {
    ...
    if (ip.ImPlot_BeginPlot("##image", .{ .x = -1, .y = 0 }, 0)) {
        //ip.ImPlot_PlotImage("my image",ig.igGetIO().*.ifa.*.TexID, st.bmin, st.bmax, st.uv0, st.uv1, st.tint, 0);
        // TODO
...
fn demo_ErrorBars() !void {
    ...
    if (ip.ImPlot_BeginPlot("##ErrorBars", .{ .x = -1, .y = 0 }, 0)) {
        ...
        // TODO
        ip.ImPlot_SetNextErrorBarStyle(.{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w }, 0, utils_ip.IMPLOT_AUTO);
...
fn demo_FilledLinePlots() !void {
    ...
    _ = ig.igCheckbox("Lines", &st.show_lines);
    ig.igSameLine(0, -1.0);
    ...
    if (ip.ImPlot_BeginPlot("Stock Prices", .{ .x = -1, .y = 0 }, 0)) {
        ...
        if (st.show_fills) {
            ...
            ipz.ImPlot_PlotShadedXyRefEx("Stock 1", &xs1, &ys1, 101, if (st.shade_mode == 0) -utils_ip.INFINITY_f32 else (if (st.shade_mode == 1) utils_ip.INFINITY_f32 else st.fill_ref), flags, 0, utils_ip.stride(xs1[0]));
            ipz.ImPlot_PlotShadedXyRefEx("Stock 2", &xs1, &ys2, 101, if (st.shade_mode == 0) -utils_ip.INFINITY_f32 else (if (st.shade_mode == 1) utils_ip.INFINITY_f32 else st.fill_ref), flags, 0, utils_ip.stride(xs1[0]));
            ipz.ImPlot_PlotShadedXyRefEx("Stock 3", &xs1, &ys3, 101, if (st.shade_mode == 0) -utils_ip.INFINITY_f32 else (if (st.shade_mode == 1) utils_ip.INFINITY_f32 else st.fill_ref), flags, 0, utils_ip.stride(xs1[0]));
...
fn demo_ScatterPlots() !void {
    ...
    if (ip.ImPlot_BeginPlot("Scatter Plot", .{ .x = -1, .y = 0 }, 0)) {
        ...
        ip.ImPlot_GetColormapColor(@ptrCast(&vec4), 1, utils_ip.IMPLOT_AUTO);
        // TODO
        ip.ImPlot_SetNextMarkerStyle(ip.ImPlotMarker_Square, 6, .{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w }, utils_ip.IMPLOT_AUTO, .{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w });
...
fn demo_Tables() !void {
    ...
    if (ig.igBeginTable("##table", 3, st.flags, .{ .x = -1, .y = 0 }, 0)) {
        ...
        for (0..10) |row| {
            ...
            // TODO
            utils_ip.Sparkline("##spark", &st.data, dtSize, 0, 11.0, @intCast(st.offset), .{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w }, .{ .x = -1, .y = 35 });
            ig.igPopID();
        }
        ip.ImPlot_PopColormap(1);
        ig.igEndTable();
...
fn demo_DragRects() !void {
    ...
    const bg_col: ig.ImVec4 =
        if (st.held) .{ .x = 0.5, .y = 0, .z = 0.5, .w = 1 } else if (st.hovered) .{ .x = 0.25, .y = 0, .z = 0.25, .w = 1 } else .{ .x = vec4.x, .y = vec4.y, .z = vec4.z, .w = vec4.w };
    // TODO
    ip.ImPlot_PushStyleColor_Vec4(ip.ImPlotCol_PlotBg, .{ .x = bg_col.x, .y = bg_col.y, .z = bg_col.z, .w = bg_col.w });
