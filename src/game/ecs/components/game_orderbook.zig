...
best_bid_cache: Price = 0,
best_ask_cache: Price = 0,

pub fn init(allocator: std.mem.Allocator, shard_count: usize) !OrderBook {
    return .{
        .allocator = allocator,
...
fn PriceLevelMap(comptime side: Side) type {
    return struct {
        ...
        pub fn reserve(self: *Self, cap: usize) !void {
            try self.levels.ensureTotalCapacity(cap);
        }
        ...
        pub fn sort(self: *Self) !void {
            ...
            switch (self.side) {
                ...
                Side.Sell => {
                    std.mem.sort(Price, self.sorted_prices.items, {}, std.sort.asc(Price));
                },
            }
            self.is_sorted = true;
        }
        ...
        pub fn getPrices(self: *Self) std.ArrayList(Price) {
            _ = self.sort() catch null;
...
pub fn getBestBid(self: *OrderBook) Price {
    if (self.best_bid_cache != 0) {
        return self.best_bid_cache;
    }

    var best_bid: Price = 0;
...
pub fn getBestAsk(self: *OrderBook) Price {
    if (self.best_ask_cache != 0) {
        return self.best_ask_cache;
    }

    var best_ask: Price = 0;
...
fn getNextBid(self: *OrderBook, current_price: Price) Price {
    ...
    // }
    return if (next_bid) |bid| bid else 0;
}

fn getNextAsk(self: *OrderBook, current_price: Price) Price {
    ...
    // }
    return if (next_ask) |ask| ask else 0;
}

fn canMatch(self: *OrderBook, side: Side, price: Price) !bool {
    switch (side) {
        Side.Buy => {
...
fn getOrdersAtPrice(self: OrderBook, side: Side, price: Price) !std.ArrayList(OrderEntry) {
...
fn matchOrder(self: *OrderBook, order_to_match: Order) !Trades {
    ...
    while (best_price != 0 and remaining_amount > 0) : (best_price = if (is_buy) self.getNextAsk(best_price) else self.getNextBid(best_price)) {
...
fn updatePriceLevel(self: *OrderBook, side: Side, price: Price, volume_delta: i64, order_count_delta: i64) !void {
...
fn updateBestPrice(self: *OrderBook, price: Price, side: Side, amount_left: usize) void {
    switch (side) {
        Side.Buy => {
            if (0 == amount_left) {
                self.best_bid_cache = self.getNextBid(self.best_bid_cache);
            }
            self.best_bid_cache = if (self.best_bid_cache != 0) @max(self.best_bid_cache, price) else price;
        },

        Side.Sell => {
            if (0 == amount_left) {
                self.best_ask_cache = self.getNextAsk(self.best_ask_cache);
            }
            self.best_ask_cache = if (self.best_ask_cache != 0) @max(self.best_ask_cache, price) else price;
        },
    }
}

fn updateMatchedOrder(self: *OrderBook, matched_order: *OrderEntry, matched_amount: Quantity, execution_price: Price) !void {
...
fn onOrderMatched(self: *OrderBook, price: Price, quantity: Quantity, is_fully_filled: bool) void {
...
pub fn priceToShard(self: *OrderBook, price: Price) usize {
    return @intCast(@mod(price, @as(i32, @intCast(self.shard_count))));
}

pub fn cancelOrder(self: *OrderBook, order: Order) OrderError!void {
...
pub fn addOrder(self: *OrderBook, order: Order) !Trades {
...
pub fn modifyOrder(self: *OrderBook, order: OrderModify) !Trades {
...
pub fn size(self: OrderBook) usize {
    return self.orders.count();
}

pub fn snapshot(self: OrderBook) !BookSnapshot {
