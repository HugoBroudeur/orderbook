...
// Only support SPIR-V
pub fn create(engine: *Engine, data: CreateShaderParam) !Shader {
    ...
    const bytecode = try getShaderByteCode(engine.allocator, engine.io, data.name);
    defer engine.allocator.free(bytecode);

    var shader: Shader = .{ .name = data.name, .entrypoint = entrypoint, .bytecode = bytecode, .stage = data.stage };

    shader.module = try engine.ctx.device.createShaderModule(&shader.moduleCreateInfo(), null);
...
fn getShaderByteCode(allocator: std.mem.Allocator, io: std.Io, name: []const u8) ![]align(@alignOf(u32)) u8 {
    var path_buf: [std.fs.max_path_bytes]u8 = undefined;
    const shader_file_path = try std.fmt.bufPrint(&path_buf, "assets/shaders/{s}", .{name});
    log.debug("Loading Shader: {s}", .{shader_file_path});

    const cwd = std.Io.Dir.cwd();
    var file = try cwd.openFile(io, shader_file_path, .{});
    defer file.close(io);

    const file_size = (try file.stat(io)).size;
    const buf = try allocator.alignedAlloc(u8, std.mem.Alignment.@"32", file_size);
