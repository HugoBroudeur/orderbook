...
pub fn loadGLTFAsset(self: *AssetManager, engine: *Engine, file_path: []const u8) !void {
    ...
    const current_img_idx = self.pool.images.items.len;
    try self.pool.images.ensureTotalCapacity(self.allocator, current_img_idx + gltf.data.images.len);
    ...
    for (gltf.data.images, 0..) |*img, i| {
        ...
        bindless_slots[i] = try engine.registerTexture(self.pool.images.items[current_img_idx + i], engine.samplers.get(.linear));
        if (Config.log.mesh) {
            log.info("[DBG] image[{d}] '{s}' → bindless slot {d}", .{ i, name, bindless_slots[i] });
        }
        try loaded_gltf.images.put(self.allocator, name, &self.pool.images.items[current_img_idx + i]);
    }
    ...
    for (gltf.data.materials, 0..) |mat, i| {
        ...
        if (mat.metallic_roughness.base_color_texture) |tex_info| {
            ...
            if (texture.source) |img_idx| {
                material_resources.color_image = self.pool.images.items[current_img_idx + img_idx];
            }
        ...
        if (mat.metallic_roughness.metallic_roughness_texture) |tex_info| {
            const texture = gltf.data.textures[tex_info.index];
            if (texture.source) |img_idx| {
                material_resources.metal_rough_image = self.pool.images.items[current_img_idx + img_idx];
            }
