...
pub const Fixed = struct {
    ...
    pub const SKIP_DRAW_THREASHOLD = 500;
    ...
    pub fn getDtSinceLastFrame(self: *Fixed) f32 {
        return @floatCast(
            @as(f64, @floatFromInt(self.threshold)) / @as(f64, @floatFromInt(self.freq)),
        );
    }

    pub fn skipDrawThreasholdReached(self: *Fixed) bool {
        return self.draw_skip >= SKIP_DRAW_THREASHOLD;
    }
};
