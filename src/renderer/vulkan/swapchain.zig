...
const SwapImage = struct {
    ...
    fn init(ctx: *const GraphicsContext, image: vk.Image, format: vk.Format) !SwapImage {
    ...
    fn deinit(self: SwapImage, ctx: *const GraphicsContext) void {
    ...
    fn waitForFence(self: SwapImage, ctx: *const GraphicsContext) !void {
        _ = try ctx.device.waitForFences(1, @ptrCast(&self.frame_fence), .true, std.math.maxInt(u64));
    }
};

fn initSwapchainImages(ctx: *const GraphicsContext, swapchain: vk.SwapchainKHR, format: vk.Format, allocator: Allocator) ![]SwapImage {
