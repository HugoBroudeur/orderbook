...
On Linux the default backend is **Vulkan**. The active entry point is `src/layers/sandbox_vulkan.zig` → `renderer/vulkan/renderer_2d.zig`.
...
| `renderer_2d.zig` | Full 2D rendering pipeline: multi-pass (demo, 2D, UI), texture sampling, batching. |
...
| `renderer_2d.zig` | Full 2D Vulkan renderer with descriptor management, frame overlap, and multi-pass rendering. |
...
Three interlocked pieces are commented out in `renderer/vulkan/renderer_2d.zig`:
